...
# Vulnerability scan: extract each pinned bottle (all four platforms, Linux
# included), SBOM with syft, match with grype. Exits 1 at/above --gate
# (default high). SBOMs + reports land in dist/scan/. Needs syft + grype on
# PATH (`nb install syft grype`).
python3 scripts/bottles/nb_bottles.py scan gh ripgrep
python3 scripts/bottles/nb_bottles.py scan --all --gate critical

# Publish the evidence: attach each SBOM + grype report to its bottle's
# manifest on GHCR as OCI referrer artifacts (also done by scan
# --push-evidence; needs write:packages). Anyone can then inspect a bottle's
# supply-chain record without trusting our CI logs:
python3 scripts/bottles/nb_bottles.py push-evidence gh
oras discover ghcr.io/justrach/nb-bottles/gh:2.91.0.linux-x86_64
# GHCR quirk (probed 2026-06-12): its OCI-1.1 referrers API is broken (303
# to a URL its own router rejects), so we maintain the spec's fallback —
# an index tagged sha256-<bottle digest> — which oras et al use transparently.
# Blob GETs 307-redirect to Azure storage: follow redirects (curl -L).

# CVE response: revoke the pinned version. Installs fall back to the previous
# known-good version (sourced from registry git history, else the upstream
# release before the pin — digests verified either way) with a warning; if no
# fallback exists installs fail closed. Propagates via `nb update-registry`
# without a binary release. unrevoke after the pin moves to a patched version.
python3 scripts/bottles/nb_bottles.py revoke gh --advisory CVE-2026-XXXX --reason "..."
python3 scripts/bottles/nb_bottles.py revoke gh --fallback-version 2.89.0 ...
python3 scripts/bottles/nb_bottles.py unrevoke gh
```

Scan caveat: Go binaries yield full dependency SBOMs (embedded buildinfo);
Rust binaries are opaque unless upstream builds with `cargo-auditable`
(uv/ruff do). For opaque binaries the GitHub-advisory data that
`update-registry` attaches to records (`security_warnings`) is the
app-level signal; the scan still catches bundled libs and Go toolchains.

## CI (`.github/workflows/bottles.yml`)

- **Weekly cron** (Mon 04:23 UTC): full `mirror` run. Idempotent — only new
  pins (after `nb update-registry` bumps) actually transfer. The same cron
  runs the **rescan job**: `scan --all --gate high` against fresh CVE data;
  a gate hit fails the job, uploads SBOMs/reports as artifacts, and opens a
  `security`-labeled issue naming the affected pins.
- **Manual dispatch**: `mirror` (optionally `--only`/dry-run), `repackage`
  (tarballs attached as a build artifact, then scanned before the run goes
  green), or `scan` (full fleet or a package list).
